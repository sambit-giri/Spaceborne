...
for probe in probes:
        ...
        # load FM
        FM_path = '/Users/davide/Documents/Lavoro/Programmi/SSC_restructured_v2/jobs/ISTF/output/FM'
        FM_GO_filename = f'FM_{probe}_GO_lmax{lmax}_nbl{nbl}_zbins{EP_or_ED:s}{zbins:02d}.txt'
        FM_GS_filename = f'FM_{probe}_GS_lmax{lmax}_nbl{nbl}_zbins{EP_or_ED:s}{zbins:02d}.txt'
        FM_GO = np.genfromtxt(f'{FM_path}/{FM_GO_filename}')
        FM_GS = np.genfromtxt(f'{FM_path}/{FM_GS_filename}')

        # get the parameters from the header and transform them into a list
        with open('/Users/davide/Documents/Lavoro/Programmi/SSC_restructured_v2/jobs/ISTF/output/FM/FM_WL_GO_lmax5000_nbl30_zbinsEP10.txt') as f:
            first_line = f.readline().lstrip('# ').strip('\n')
        try:
            paramnames_FM = json.loads(first_line)
        except:
            paramnames_FM = json.loads(first_line.replace('\'', '"'))

        wzwa_rows = paramnames_FM.index('wz'), paramnames_FM.index('wa')
        try:
            ox_row = paramnames_FM.index('Ox')
        except:
            print('Ox not found in paramnames_FM')
        ...
        for FM, case in zip(FMs, cases):
            ...
            fom[case] = mm.compute_FoM(FM, w0wa_rows=wzwa_rows)
