...
plt.figure()
# HM
plt.plot(k_grid_dPk_hm, np.abs(dPmm_ddeltab_hm[:, z_idx_hm]), label=f'dPmm_ddeltab_hm, z={z_val_hm}', ls='--', alpha=0.5, c='tab:blue')
plt.plot(k_grid_dPk_hm, np.abs(dPgm_ddeltab_hm[:, z_idx_hm]), label=f'dPgm_ddeltab_hm, z={z_val_hm}', ls='--', alpha=0.5, c='tab:orange')
plt.plot(k_grid_dPk_hm, np.abs(dPgg_ddeltab_hm[:, z_idx_hm]), label=f'dPgg_ddeltab_hm, z={z_val_hm}', ls='--', alpha=0.5, c='tab:green')

# SU
plt.plot(sscint.k_grid_dPk, np.abs(sscint.dPmm_ddeltab[:, z_idx_su]), label=f'dPmm_ddeltab, z={z_val_su:.2f}', alpha=0.5, c='tab:blue')
plt.plot(sscint.k_grid_dPk, np.abs(sscint.dPgm_ddeltab[:, z_idx_su]),label=f'dPgm_ddeltab, z={z_val_su:.2f}', alpha=0.5, c='tab:orange')
plt.plot(sscint.k_grid_dPk, np.abs(sscint.dPgg_ddeltab[:, z_idx_su]), label=f'dPgg_ddeltab, z={z_val_su:.2f}', alpha=0.5, c='tab:green')
...
lines = [plt.Line2D([], [], color='k', linestyle=ls) for ls in ['--', '-']]
plt.legend(lines, ['HM', 'SU'], bbox_to_anchor=(1, 1))
plt.show()
...
for count, z_val in enumerate((0, 0.5, 1, 2, 3)):
    ...
    z_idx_su = np.argmin(np.abs(sscint.z_grid_dPk - z_val))
    z_idx_hm = np.argmin(np.abs(z_grid_dPk_hm - z_val))

    z_hm = z_grid_dPk_hm[z_idx_hm]
    z_su = sscint.z_grid_dPk[z_idx_su]

    plt.loglog(sscint.k_grid_dPk, sscint.pk_mm_2d[:, z_idx_su], ls='-', c=clr[count], label='cloe')
...
plt.plot(sscint.k_grid_dPk, sscint.dPmm_ddeltab[:, z_idx_su], label=f'dPmm_ddeltab, z={z_su:.2f}', alpha=0.5, c='tab:blue')
plt.plot(sscint.k_grid_dPk, sscint.dPgm_ddeltab[:, z_idx_su], label=f'dPgm_ddeltab, z={z_su:.2f}', alpha=0.5, c='tab:orange')
plt.plot(sscint.k_grid_dPk, sscint.dPgg_ddeltab[:, z_idx_su], label=f'dPgg_ddeltab, z={z_su:.2f}', alpha=0.5, c='tab:green')
plt.legend()
plt.xscale('log')
plt.xlabel('k [1/Mpc]')
