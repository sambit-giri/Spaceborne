...
ell_min_list = (10, 50, 100, 200, 500, 1000, 1500, 2000, 2500)
for general_cfg['ell_min'] in ell_min_list:
...
# ! populate fm dict with ell_min:
plt.figure()
for probe_here in ['WL', 'GC', '3x2pt']:
    for go_or_gs_here in ['GO', 'GS']:
        for ell_min in ell_min_list:
            lmax = general_cfg[f'ell_max_{probe}'] if probe in ['WL', 'GC'] else general_cfg['ell_max_XC']

            fm_folder = FM_cfg["fm_folder"].format(SSC_code='PySSC')
            fm_name = FM_cfg["FM_txt_filename"].format(probe=probe_here, which_cov=go_or_gs_here, ell_min=ell_min,
                                                       ell_max=lmax, nbl=nbl, EP_or_ED='EP', zbins=10)
            FM_dict[f'FM_PySSC_{probe_here}_{go_or_gs_here}_lmin{ell_min}'] = np.genfromtxt(
                f'{fm_folder}/{fm_name}.txt')
...
# plot uncert ell_min:
probe = '3x2pt'
ell_min_list = (10, 50, 100, 500, 1000)
perc_diff = np.zeros((len(ell_min_list), nparams_toplot))
go_arr = np.zeros((len(ell_min_list), nparams_toplot))
gs_arr = np.zeros((len(ell_min_list), nparams_toplot))
for ell_min_idx, ell_min in enumerate(ell_min_list):
    perc_diff[ell_min_idx, :] = mm.percent_diff(uncert_dict[f'FM_PySSC_{probe}_GS_lmin{ell_min}'],
                                                uncert_dict[f'FM_PySSC_{probe}_GO_lmin{ell_min}'])
    go_arr[ell_min_idx, :] = uncert_dict[f'FM_PySSC_{probe}_GO_lmin{ell_min}']
    gs_arr[ell_min_idx, :] = uncert_dict[f'FM_PySSC_{probe}_GS_lmin{ell_min}']

plt.figure()
colors = cm.rainbow(np.linspace(0, 1, nparams_toplot))
plt.title(probe)
for param_idx in range(nparams_toplot):
    plt.plot(ell_min_list, perc_diff[:, param_idx], c=colors[param_idx], label=param_names_list[param_idx])
    # plt.plot(ell_min_list, go_arr[:, param_idx], c=colors[param_idx], label=param_names_list[param_idx])
    # plt.plot(ell_min_list, gs_arr[:, param_idx], ls='--', c=colors[param_idx], label=param_names_list[param_idx])
plt.xlabel('ell_min')
plt.ylabel('GS/GO - 1 [%]')
plt.legend()
