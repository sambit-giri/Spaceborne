...
# link to notebook: https://gitlab.euclid-sgs.uk/pf-ist-nonlinear/likelihood-implementation/-/blob/develop_NL_emu/notebooks/DEMO_ISTNL_photo.ipynb
# cells 29-30
...
for NL_flag in range(NL_flags):
    ...
    if NL_flag == 0: # linear: store in NL = 0 index
        cov_WL_GO_2D[NL_flag, ...] = np.load(job_path / f'output/covmat/CovMat-ShearShear-Gauss-20bins.npy')
        cov_WL_GS_2D[NL_flag, ...] = np.load(job_path / f'output/covmat/CovMat-ShearShear-GaussSSC-20bins.npy')
    else: # nonlinear
        cov_WL_GO_2D[NL_flag, ...] = np.load(job_path / f'output/covmat/CovMat-ShearShear-Gauss-20bins-NL_flag_{NL_flag}.npy')
        cov_WL_GS_2D[NL_flag, ...] = np.load(job_path / f'output/covmat/CovMat-ShearShear-GaussSSC-20bins-NL_flag_{NL_flag}.npy')
...
# subtract Gaussian cov from the diagonal:
cov_WL_mix_4D = np.zeros((NL_flags, nbl, nbl, n_zpairs, n_zpairs))
cov_WL_SS_4D = np.zeros((NL_flags, nbl, nbl, n_zpairs, n_zpairs))

cov_WL_mix_2D = np.zeros((NL_flags, nbl*n_zpairs, nbl*n_zpairs))
cov_WL_SS_2D = np.zeros((NL_flags, nbl*n_zpairs, nbl*n_zpairs))

for NL_flag in range(5):
    # SS-only covariance: GS-GO
    cov_WL_SS_4D[NL_flag, ...] = cov_WL_GS_4D[NL_flag, ...] - cov_WL_GO_4D[NL_flag, ...]
    ...
    cov_WL_SS_2D[NL_flag, ...] = mm.cov_4D_to_2D(cov_WL_SS_4D[NL_flag, ...], nbl, n_zpairs, block_index='vincenzo')
...
# test plot with errorbars - this is one of the 4 panels shown in the multi-panel plot below
...
SS_samples_thinned = np.copy(GO_samples_thinned)
mix_samples_thinned = np.copy(GO_samples_thinned)

GO_samples_in_range = np.copy(GO_samples_thinned)
GS_samples_in_range = np.copy(GO_samples_thinned)
SS_samples_in_range = np.copy(GO_samples_thinned)
mix_samples_in_range = np.copy(GO_samples_thinned)

# store the samples in the analytical confidence interval
GO_in_conf_interval = []
GS_in_conf_interval = []

# sampled covariance, check that it's equal to the analytical covariance (suggested
# by Martin)
cov_sampled_GO_2D = np.zeros((NL_flags, cov_WL_GO_2D.shape[1], cov_WL_GO_2D.shape[1]))
...
for NL_flag in range(NL_flags):
    ...
    # take only the elements corresponding to the various iz_idx values
    for cur_ell in range(nbl):
        for iz_idx in range(4):
            ...
            # take only the iz_idx elements (4 in total) of each sample expand ell x ij (right side) 
            # into (iz_idx, ell) (left side)
            GO_samples_thinned[NL_flag, iz_idx, cur_ell, :] = GO_samples[NL_flag, :, cur_ell*n_zpairs + zpair]
            GS_samples_thinned[NL_flag, iz_idx, cur_ell, :] = GS_samples[NL_flag, :, cur_ell*n_zpairs + zpair]
            SS_samples_thinned[NL_flag, iz_idx, cur_ell, :] = SS_samples[NL_flag, :, cur_ell*n_zpairs + zpair]
            mix_samples_thinned[NL_flag, iz_idx, cur_ell, :] = mix_samples[NL_flag, :, cur_ell*n_zpairs + zpair]
    ...
    for cur_ell in range(nbl):
        for iz_idx in range(4):
            ...
            np.logical_and(GO_samples_thinned[NL_flag, iz_idx, cur_ell, :] < ClWL_1D[cur_ell*n_zpairs + zpair] + sigma_WL_GO[NL_flag, iz_idx, cur_ell],\
                           GO_samples_thinned[NL_flag, iz_idx, cur_ell, :] > ClWL_1D[cur_ell*n_zpairs + zpair] - sigma_WL_GO[NL_flag, iz_idx, cur_ell])
            GS_samples_in_range[NL_flag, iz_idx, cur_ell, :] =\
            np.logical_and(GS_samples_thinned[NL_flag, iz_idx, cur_ell, :] < ClWL_1D[cur_ell*n_zpairs + zpair] + sigma_WL_GS[NL_flag, iz_idx, cur_ell],\
                           GS_samples_thinned[NL_flag, iz_idx, cur_ell, :] > ClWL_1D[cur_ell*n_zpairs + zpair] - sigma_WL_GS[NL_flag, iz_idx, cur_ell])
...
figure = corner.corner(GO_samples_thinned[NL_flag, iz_idx_test, :stop, :].transpose(),
                       quantiles=[0.16, 0.5, 0.84], label='GO_samples',
                       show_titles=True, title_kwargs={"fontsize": 12})
# figure = corner.corner(GS_samples_thinned[NL_flag, iz_idx_test, :stop, :].transpose(), fig = figure,
#                        quantiles=[NL_flag, 0.16, 0.5, 0.84], label='GS_samples',
#                        show_titles=False, title_kwargs={"fontsize": 12}, color = 'red')
figure = corner.corner(mix_samples_thinned[NL_flag, iz_idx_test, :stop, :].transpose(), fig = figure,
                        quantiles=[0.16, 0.5, 0.84], label='mix_samples',
                        show_titles=False, title_kwargs={"fontsize": 12}, color = 'red')
...
plt.figure()
plt.hist(GO_samples_thinned[NL_flag, iz_idx_test, ell_test, :], alpha=0.6, bins=40)
# plt.hist(GS_samples_thinned[NL_flag, iz_idx_test, ell_test, :], alpha=0.6, bins=40)
plt.hist(mix_samples_thinned[NL_flag, iz_idx_test, ell_test, :], alpha=0.6, bins=40)

# plot of gaussian of analytical cov:
x = np.linspace(ClWL_1D[NL_flag, :] - 3*sigma, ClWL_1D[NL_flag, :] + 3*sigma, 100)
plt.plot(x, stats.norm.pdf(x, ClWL_1D[NL_flag, :], sigma))
...
labels = ['Linear Recipe', 'Halofit', 'Mead2020', 'Euclid Emulator', 'Bacco Recipe']
...
labels = list(itemgetter(*NL_values)(labels))
linewidth = 1
...
for iz_idx, iz in enumerate(iz_values):
    ...
    for NL_flag, pl_label in zip(NL_values, labels):
        ...
        if which_plot == 'absolute':
            ax1.loglog(ell_values, ClWL_3D_list, color=colors[NL_flag], ls='--', label=f'{pl_label}', zorder=2, lw=linewidth)
      
        # plot the ratios to halofit
        elif which_plot == 'ratio_to_halofit':
            ax1.loglog(ell_values, ClWL_3D_ratio_list, color=colors[NL_flag], ls='--', label=f'{pl_label}', zorder=2, lw=linewidth)
            # compute uncertainties on the ratio - formula from https://en.wikipedia.org/wiki/Propagation_of_uncertainty
        ...
        if (NL_flag == 4) and plot_uncertainties:
            if which_plot == 'absolute':
                ax1.fill_between(ell_values, ClWL_3D_list - sigma_WL_GS_list, ClWL_3D_list + sigma_WL_GS_list, label = 'GaussSSC' if NL_flag == 4 else "", color='blue', alpha=0.2, zorder=1)
                ax1.fill_between(ell_values, ClWL_3D_list - sigma_WL_GO_list, ClWL_3D_list + sigma_WL_GO_list, label = 'Gauss-only'if NL_flag == 4 else "", color='blue', alpha=0.3, zorder=1)
            # if I'm plotting the ratio, I need to change the Cls
            elif which_plot == 'ratio_to_halofit':
                ax1.fill_between(ell_values, ClWL_3D_ratio_list - sigma_WL_GS_list, ClWL_3D_ratio_list + sigma_WL_GS_list, label = 'GaussSSC' if NL_flag == 4 else "", color='blue', alpha=0.2, zorder=1)
                ax1.fill_between(ell_values, ClWL_3D_ratio_list - sigma_WL_GO_list, ClWL_3D_ratio_list + sigma_WL_GO_list, label = 'Gauss-only'if NL_flag == 4 else "", color='red', alpha=0.3, zorder=1)
