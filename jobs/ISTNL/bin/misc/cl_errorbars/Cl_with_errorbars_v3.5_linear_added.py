...
for NL_flag in range(NL_flags):
    ...
    # reshape to 6D
    cov_WL_GO_6D[NL_flag, ...] = mm.cov_4D_to_6D(cov_WL_GO_4D[NL_flag, ...], nbl, zbins, probe='LL', ind=ind_CLOE[:55, :])
    cov_WL_GS_6D[NL_flag, ...] = mm.cov_4D_to_6D(cov_WL_GS_4D[NL_flag, ...], nbl, zbins, probe='LL', ind=ind_CLOE[:55, :])
    cov_WL_SS_6D[NL_flag, ...] = mm.cov_4D_to_6D(cov_WL_SS_4D[NL_flag, ...], nbl, zbins, probe='LL', ind=ind_CLOE[:n_zpairs, :])
    ...
    cov_WL_mix_6D[NL_flag, ...] = mm.cov_4D_to_6D(cov_WL_mix_4D[NL_flag, ...], nbl, zbins, probe='LL', ind=ind_CLOE[:n_zpairs, :])


# check cov_mix
NL_flag_test = 1
mm.matshow(cov_WL_mix_2D[NL_flag_test, ...], log=True, title='cov_WL_mix_2D')
mm.matshow(cov_WL_GS_2D[NL_flag_test, ...], log=True, title='cov_WL_GS_2D')
mm.matshow(cov_WL_GO_2D[NL_flag_test, ...], log=True, title='cov_WL_GO_2D')

# TODO check this? why does this give "true?"
mm.check_array_equality(cov_WL_GS_4D[NL_flag_test, 55:110, 0:55, ...], cov_WL_mix_2D[NL_flag_test, 55:110, 0:55])
...
for NL_flag in range(NL_flags):
    for iz_idx, iz in enumerate(iz_values):
        for ell_idx, _ in enumerate(ell_values):
            sigma_WL_GO[NL_flag, ell_idx, iz_idx] = np.sqrt(cov_WL_GO_6D[NL_flag, ell_idx, ell_idx, iz - 1, iz - 1, iz - 1, iz - 1])
            sigma_WL_GS[NL_flag, ell_idx, iz_idx] = np.sqrt(cov_WL_GS_6D[NL_flag, ell_idx, ell_idx, iz - 1, iz - 1, iz - 1, iz - 1])
            sigma_WL_SS[NL_flag, ell_idx, iz_idx] = np.sqrt(cov_WL_SS_6D[NL_flag, ell_idx, ell_idx, iz - 1, iz - 1, iz - 1, iz - 1])
            sigma_WL_mix[NL_flag, ell_idx, iz_idx] = np.sqrt(cov_WL_mix_6D[NL_flag, ell_idx, ell_idx, iz - 1, iz - 1, iz - 1, iz - 1])
...
sigma_WL_GO_sampled_2D = np.zeros((NL_flags, nbl, n_zpairs))
sigma_WL_GS_sampled_2D = np.copy(sigma_WL_GO_sampled_2D)
sigma_WL_SS_sampled_2D = np.copy(sigma_WL_GO_sampled_2D)
sigma_WL_mix_sampled_2D = np.copy(sigma_WL_GO_sampled_2D)
...
for NL_flag in range(NL_flags):
    ...
    sigma_WL_GO_sampled_2D[NL_flag, :, :] = np.sqrt(np.var(GO_samples_2D[NL_flag, :, :], axis=2))
    sigma_WL_GS_sampled_2D[NL_flag, :, :] = np.sqrt(np.var(GS_samples_2D[NL_flag, :, :], axis=2))
    sigma_WL_SS_sampled_2D[NL_flag, :, :] = np.sqrt(np.var(SS_samples_2D[NL_flag, :, :], axis=2))
    sigma_WL_mix_sampled_2D[NL_flag, :, :] = np.sqrt(np.var(mix_samples_2D[NL_flag, :, :], axis=2))
...
plt.plot(range(nbl), sigma_WL_SS[NL_flag, :, iz_idx_test] + sigma_WL_GO[NL_flag, :, iz_idx_test], label='GO + SS')
...
for iz_idx, iz in enumerate(iz_values):
    ...
    for NL_flag, pl_label in zip(NL_values, NL_labels):
        ...
        if (NL_flag == 4 or NL_flag == 0) and plot_uncertainties:
            if which_plot == 'absolute':
                ax1.fill_between(ell_values, ClWL_3D_list - sigma_WL_GO_list, ClWL_3D_list + sigma_WL_GO_list,
                                 label='Gauss-only' if NL_flag == 4 else "", color='red', alpha=0.3, zorder=1)
                ax1.fill_between(ell_values, ClWL_3D_list - sigma_WL_GS_list, ClWL_3D_list + sigma_WL_GS_list,
                                 label='GaussSSC' if NL_flag == 4 else "", color='blue', alpha=0.2, zorder=1)
                for (sigma, label, color) in zip([sigma_WL_GS_list, sigma_WL_GO_list], ['GaussSSC', 'Gauss-only',], ['blue', 'red']):
                    ax1.fill_between(ell_values, ClWL_3D_ratio_list - sigma, ClWL_3D_ratio_list + sigma, 
                                      label=label if NL_flag == 4 else "", color=color, alpha=0.2, zorder=1)

            # if I'm plotting the ratio, I need to change the Cls
            elif which_plot == 'ratio_to_halofit':
                # ax1.fill_between(ell_values, ClWL_3D_ratio_list - sigma_WL_GO_list, ClWL_3D_ratio_list + sigma_WL_GO_list, 
                #                   label='Gauss-only' if NL_flag == 4 else "", color='red', alpha=0.3, zorder=1)
                # ax1.fill_between(ell_values, ClWL_3D_ratio_list - sigma_WL_GS_list, ClWL_3D_ratio_list + sigma_WL_GS_list, 
                #                   label='GaussSSC' if NL_flag == 4 else "", color='blue', alpha=0.2, zorder=1)
...
plt.savefig(job_path / f'output/Cl_errorbars/{which_plot}_plot_uncer_{plot_uncertainties}_{which_uncertainties}.png', dpi=300)
