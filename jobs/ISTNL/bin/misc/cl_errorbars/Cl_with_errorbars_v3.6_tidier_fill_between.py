...
for NL_flag in range(NL_flags):
    ...
    cov_WL_mix_6D[NL_flag, ...] = mm.cov_4D_to_6D(cov_WL_mix_4D[NL_flag, ...], nbl, zbins, probe='LL',
                                                  ind=ind_CLOE[:n_zpairs, :])
...
#     for ell2 in range(nbl):
#         if ell1 != ell2:
#             print(np.all(cov_WL_GS_4D[NL_flag_test, ell1, ell2, ...] == cov_WL_mix_4D[NL_flag_test, ell1, ell2, ...]))
# the issue may be when reshaping cov_mix to 2D? It's the only thing I do differently

print(np.all(cov_WL_mix_2D[NL_flag_test]))
...
for NL_flag in range(NL_flags):
    ...
    for ell in range(nbl):
        for iz_idx, iz in enumerate(iz_expanded):
            ...
            sigma_WL_GO_sampled_thinned_2D[NL_flag, :, :] = np.sqrt(
                np.var(GO_samples_thinned_2D[NL_flag, :, :], axis=2))
            sigma_WL_GS_sampled_thinned_2D[NL_flag, :, :] = np.sqrt(
                np.var(GS_samples_thinned_2D[NL_flag, :, :], axis=2))
            sigma_WL_SS_sampled_thinned_2D[NL_flag, :, :] = np.sqrt(
                np.var(SS_samples_thinned_2D[NL_flag, :, :], axis=2))
            sigma_WL_mix_sampled_thinned_2D[NL_flag, :, :] = np.sqrt(
                np.var(mix_samples_thinned_2D[NL_flag, :, :], axis=2))
