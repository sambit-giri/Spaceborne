...
def consistency_checks(general_config, covariance_config):
    ...
    assert covariance_config['fsky'] == 0.3636102718447355, 'IST_NL uses fsky = 0.3636'
    ...
    assert general_config['Cij_folder'] == 'Cij_14may', 'not quite sure about this Cij_folder thing...'
    assert general_config['use_WA'] == False, 'IST_NL doesn not use Wadd'
...
def import_and_interpolate_cls(general_config, ell_dict):
    ...
    if Cij_folder == "Cij_thesis":
        C_LL_import = np.genfromtxt(job_path / 'input/vincenzo/thesis_data/Cij_tesi/CijGG-N4TB-GR-eNLA.dat')
        C_XC_import = np.genfromtxt(job_path / 'input/vincenzo/thesis_data/Cij_tesi/CijDG-N4TB-GR-eNLA.dat')
        C_GG_import = np.genfromtxt(job_path / 'input/vincenzo/thesis_data/Cij_tesi/CijDD-N4TB-GR-eNLA.dat')
    elif Cij_folder == "Cij_15gen":  # Cij-NonLin-eNLA_15gen
        C_LL_import = np.genfromtxt(job_path / 'input/vincenzo/Cij-NonLin-eNLA_15gen/CijLL-LCDM-NonLin-eNLA.dat')
        C_XC_import = np.genfromtxt(job_path / 'input/vincenzo/Cij-NonLin-eNLA_15gen/CijLG-LCDM-NonLin-eNLA.dat')
        C_GG_import = np.genfromtxt(job_path / 'input/vincenzo/Cij-NonLin-eNLA_15gen/CijGG-LCDM-NonLin-eNLA.dat')
    ...
    if Cij_folder == "Cij_14may":
        C_LL_import = np.genfromtxt(job_path / 'input/vincenzo/CijDers/14may/EP10/CijLL-GR-Flat-eNLA-NA.dat')
        C_XC_import = np.genfromtxt(
            job_path / 'input/vincenzo/CijDers/14may/EP10/CijGL-GR-Flat-eNLA-NA.dat')  # XXX GL, not LG!!
        C_GG_import = np.genfromtxt(job_path / 'input/vincenzo/CijDers/14may/EP10/CijGG-GR-Flat-eNLA-NA.dat')
