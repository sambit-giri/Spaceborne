...
def consistency_checks(general_config, covariance_config):
    ...
    """
    assert covariance_config['fsky'] == 0.3636102718447355, 'IST forecast uses fsky = 0.3636'
    ...
    assert general_config['Cij_folder'] == 'Cij_14may', 'Latest Cls are Cij_14may'
    assert general_config['use_WA'], 'IST uses Wadd'
...
def import_and_interpolate_cls(general_config, ell_dict):
    ...
    if Cij_folder == "Cij_14may":
        ...
        C_XC_import = np.genfromtxt(
            path / 'input/vincenzo/CijDers/14may/EP10/CijGL-GR-Flat-eNLA-NA.dat')  # XXX GL, not LG!!
