...
for general_cfg['ell_max_WL'], general_cfg['ell_max_GC'] in (5000, 3000), (1500, 750):
    ...
    ind = np.genfromtxt(project_path.parent / f"common_data/ind_files/backup/indices_{ind_ordering}_like_int.dat", dtype=int)
    ...
    # reshape them to 3D
    cl_dict_2D['cl_GL_2D'] = cl_dict_2D['cl_XC_2D']  # just for Cij14may
    rl_dict_2D['rl_GL_2D'] = rl_dict_2D['rl_XC_2D']  # just for alex's respondses
...
covariance_cfg['which_probe_response'] = 'constant'
tests.check_FMs_against_oldSSCscript(f'{job_path}/output/FM', general_cfg, covariance_cfg, tolerance=0.0001)
