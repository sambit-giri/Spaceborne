...
for kmax in general_cfg['kmax_list']:
    for general_cfg['magcut_lens'], general_cfg['zcut_lens'], \
        ...
        if general_cfg['ell_cuts']:
            assert general_cfg['BNT_transform'], 'ell_cuts should only be implemented with the BNT_transform set to True'
        ...
        warnings.warn(f'probably not the best idea to let the main write on the config file...')
        general_cfg['nbl_WL'] = nbl_WL
        general_cfg['nbl_GC'] = nbl_GC
        general_cfg['nbl_WA'] = nbl_WA
        ...
        if FM_cfg['compute_FM']:
            ...
            if general_cfg['deriv_BNT_transform']:
                ...
                if general_cfg['ell_cuts']:
                    ...
                    kmax_ref = 1.  # h/Mpc; the one with which the above ell cuts were computed
                    ell_cuts_LL *= kmax/kmax_ref
                    ell_cuts_GG *= kmax/kmax_ref
                    ell_cuts_XC *= kmax/kmax_ref
                    ...
                    dC_LL_4D_v2 = dC_LL_4D.copy()
                    dC_WA_4D_v2 = dC_WA_4D.copy()
                    dC_GG_4D_v2 = dC_GG_4D.copy()
                    for param_idx in range(len(paramnames_3x2pt)):
                        dC_LL_4D[:, :, :, param_idx] = func(dC_LL_4D[:, :, :, param_idx], ell_cuts_LL, ell_dict['ell_WL'])
                        dC_WA_4D[:, :, :, param_idx] = func(dC_WA_4D[:, :, :, param_idx], ell_cuts_LL, ell_dict['ell_WA'])
                        dC_GG_4D[:, :, :, param_idx] = func(dC_GG_4D[:, :, :, param_idx], ell_cuts_GG, ell_dict['ell_GC'])
            ...
            FM_utils.save_FM(FM_dict, FM_cfg, save_txt=FM_cfg['save_FM_txt'], save_dict=FM_cfg['save_FM_dict'],
                             **variable_specs)
