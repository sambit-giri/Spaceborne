...
for kmax_h_over_Mpc in general_cfg['kmax_h_over_Mpc_list']:
    for general_cfg['which_cuts'] in ['Vincenzo', ]:
        for general_cfg['center_or_min'] in ['min', ]:
            ...
            fiducials_dict_flattened = {param_names_3x2pt[i]: fiducials_3x2pt[i] for i in
            ...
            if FM_cfg['load_preprocess_derivatives']:
                ...
                np.save(f'{derivatives_folder}/reshaped_into_4d_arrays/dC_LL_4D.npy', dC_LL_4D)
                np.save(f'{derivatives_folder}/reshaped_into_4d_arrays/dC_GG_4D.npy', dC_GG_4D)
                np.save(f'{derivatives_folder}/reshaped_into_4d_arrays/dC_WA_4D.npy', dC_WA_4D)
                np.save(f'{derivatives_folder}/reshaped_into_4d_arrays/dC_3x2pt_6D.npy', dC_3x2pt_6D)
            ...
            fm_folder = FM_cfg['fm_folder'].format(ell_cuts=str(general_cfg['ell_cuts']),
                                                   ...
                                                   center_or_min=general_cfg['center_or_min'],
                                                   ell_cuts_subfolder=general_cfg['ell_cuts_subfolder'])

            if not general_cfg['ell_cuts']:
                fm_folder = fm_folder.replace(f'/{general_cfg["which_cuts"]}/ell_{center_or_min}', '')
                FM_cfg['FM_txt_filename'] = FM_cfg['FM_txt_filename'].replace('_kmaxhoverMpc{kmax_h_over_Mpc:03f}', '')
                FM_cfg['FM_dict_filename'] = FM_cfg['FM_dict_filename'].replace('_kmaxhoverMpc{kmax_h_over_Mpc:03f}', '')
