...
def generate_Cls(general_settings, ell_dict, bia):
    """
    ...
    if Cij_folder == "Cij_14may":
        ...
        # take ell in lin scale, Santiago uses this
        C_LL_import[:,0] = 10**(C_LL_import[:,0])
        C_XC_import[:,0] = 10**(C_XC_import[:,0])
        C_GG_import[:,0] = 10**(C_GG_import[:,0])
    if Cij_folder == 'Cij_marco':
        print('bia =', bia)
        C_LL_import = np.load(path / f'data/CosmoCentral_outputs/Cl/C_LL_3D_marco_bia{bia}.npy')
        C_XC_import = np.load(path / f'data/CosmoCentral_outputs/Cl/C_GL_3D_marco_bia{bia}.npy') # XXX GL, not LG!!
        C_GG_import = np.load(path / f'data/CosmoCentral_outputs/Cl/C_GG_3D_marco_bia{bia}.npy')
        ell_marco = np.load(path / 'data/CosmoCentral_outputs/ell_values_marco.npy')
        # make 2D 
        C_LL_import = mm.Cl_3D_to_2D_symmetric(C_LL_import, nbl=C_LL_import.shape[0], npairs=55, zbins=10)
        C_XC_import = mm.Cl_3D_to_2D_asymmetric(C_XC_import, nbl=C_XC_import.shape[0], npairs=100, zbins=10)
        C_GG_import = mm.Cl_3D_to_2D_symmetric(C_GG_import, nbl=C_LL_import.shape[0], npairs=55, zbins=10)
        # add column of ell values 
        C_LL_import = np.insert(C_LL_import, 0, ell_marco, axis=1)
        C_XC_import = np.insert(C_XC_import, 0, ell_marco, axis=1)
        C_GG_import = np.insert(C_GG_import, 0, ell_marco, axis=1)
    ...
    # take santiago's Cls
    # C_LL_WLonly_2D = Cl_dict_2D['C_LL_2D']
    # C_LL_2D = Cl_dict_2D['C_LL_2D']
    # C_GG_2D = Cl_dict_2D['C_GG_2D']
    # C_XC_2D = Cl_dict_2D['C_GL_2D']
