...
def generate_Cls(nbl, ell_max_WL, ell_max_GC,
                 zbins, nProbes, ind_ordering,
                 ell_dict, Cij_folder):
    ...
    D_ALL = np.zeros((nbl, nProbes, nProbes, zbins, zbins))
    ...
    D_ALL[:, 0, 0, :, :] = C_LL_3D
    D_ALL[:, 1, 1, :, :] = C_GG_3D
    D_ALL[:, 0, 1, :, :] = np.transpose(C_XC_3D, (0,2,1)) # XXX pay attention to LG, GL...
    D_ALL[:, 1, 0, :, :] = C_XC_3D # XXX pay attention to LG, GL...
