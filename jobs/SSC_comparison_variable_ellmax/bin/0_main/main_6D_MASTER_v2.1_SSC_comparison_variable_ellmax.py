...
# job utils
import utils_SSC_comparison_variable_ellmax as utils
...
ell_max_WL_values = np.linspace(750, 5000, 10)
ell_max_GC_values = np.copy(ell_max_WL_values)


for general_config['ell_max_WL'], general_config['ell_max_GC'] in zip(ell_max_WL_values, ell_max_GC_values):

    print(f"working on (ell_max_WL, ell_max_GC) = ({general_config['ell_max_WL']:.2f}, {general_config['ell_max_GC']:.2f})")
    ...
    if FM_config['save_FM']:
        np.savetxt(job_path / f"output/FM/FM_WL_GO_lmaxWL{ell_max_WL:.2f}_nbl{nbl}.txt", FM_dict['FM_WL_GO'])
        np.savetxt(job_path / f"output/FM/FM_GC_GO_lmaxGC{ell_max_GC:.2f}_nbl{nbl}.txt", FM_dict['FM_GC_GO']) 
        np.savetxt(job_path / f"output/FM/FM_3x2pt_GO_lmaxXC{ell_max_XC:.2f}_nbl{nbl}.txt", FM_dict['FM_3x2pt_GO']) 

        np.savetxt(job_path / f"output/FM/FM_WL_GS_lmaxWL{ell_max_WL:.2f}_nbl{nbl}.txt", FM_dict['FM_WL_GS'])
        np.savetxt(job_path / f"output/FM/FM_GC_GS_lmaxGC{ell_max_GC:.2f}_nbl{nbl}.txt", FM_dict['FM_GC_GS'])
        np.savetxt(job_path / f"output/FM/FM_3x2pt_GS_lmaxXC{ell_max_XC:.2f}_nbl{nbl}.txt", FM_dict['FM_3x2pt_GS'])
